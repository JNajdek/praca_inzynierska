...
class MyFrame(wx.Frame):
    def __init__(self, *args, **kwds):
        ...
        self.combo_box_1 = wx.ComboBox(sizer_9.GetStaticBox(), wx.ID_ANY, choices=["Visualise current recording session", "Visualise last recording session", "Select the directory from operating system", ""], style=wx.CB_READONLY)
        ...
        self.button_acc = wx.ToggleButton(self.panel_2, wx.ID_ANY, " Amplitude\nChannnels Combined")
        sizer_19.Add(self.button_acc, 0, wx.ALIGN_CENTER_VERTICAL | wx.ALL, 10)

        self.button_ht = wx.ToggleButton(self.panel_2, wx.ID_ANY, "Heatmap\n")
        ...
        self.button_vs = wx.ToggleButton(self.panel_3, wx.ID_ANY, "Visualize Standard")
        sizer_15.Add(self.button_vs, 0, wx.ALIGN_CENTER_VERTICAL | wx.ALL, 10)

        self.button_vsf = wx.ToggleButton(self.panel_3, wx.ID_ANY, "Visualize Selected \nFeatures")
        sizer_15.Add(self.button_vsf, 0, wx.ALIGN_CENTER_VERTICAL | wx.ALL, 10)

        self.button_vsd = wx.ToggleButton(self.panel_3, wx.ID_ANY, "Visualize Selected\n Distributions")
        sizer_15.Add(self.button_vsd, 0, wx.ALIGN_CENTER_VERTICAL | wx.ALL, 10)

        self.button_vsds = wx.ToggleButton(self.panel_3, wx.ID_ANY, "Visualize Selected\n Distributions Separately")
        ...
        if self.button_10.GetValue():
            ...
            if self.combo_choice == "Select the directory from operating system" or self.combo_choice == "Visualise last recording session":
                if self.button_vs.GetValue():
                    feature_set_dict, fe = self.create_visaulization_parameters_offline(['MEAN'], "Standard")
                    fe.visualize(feature_set_dict)
                if self.button_vsf.GetValue():
                    feature_set_dict, fe = self.create_visaulization_parameters_offline(self.selected_features)
                    fe.visualize(feature_set_dict)
                if self.button_vsd.GetValue():
                    feature_set_dict, fe = self.create_visaulization_parameters_offline(self.selected_features_dist)
                    fe.visualize(feature_set_dict)
                if self.button_vsds.GetValue():
                    feature_set_dict, fe = self.create_visaulization_parameters_offline(self.selected_features_dist_sep)
                    fe.visualize(feature_set_dict)
...
# end of class MyFrame
    ...
    def create_visaulization_parameters_offline(self, features_list, mode=None):
        ofdh = OfflineDataHandler()
        ofdh.get_data(folder_location=self.folder_visualize, regex_filters=self.regex_filters, delimiter=' ')
        ofdh_new = ofdh.isolate_channels([x + 1 for x in self.active_sensors])
        if mode == 'Standard':
