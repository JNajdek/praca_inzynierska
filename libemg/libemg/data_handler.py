    ...
    def _log_to_file(self):
        ...
        while True:
            ...
            for m in vals.keys():
                ...
                #new_samples[np.abs(new_samples) < 1e-5] = 0.0
                ...
                if num_new_samples:
                    ...
                    if self.timestamps:
                        ...
                        #new_samples[np.abs(new_samples) < 1e-5] = 0.0
                        #print(np.hstack((np.ones((new_samples.shape[0],1))*timestamp, new_samples)))
                        np.savetxt(files[m], np.hstack((np.ones((new_samples.shape[0],1))*timestamp, new_samples)), fmt='%.4f')
                        # check to see if they're in the right order, or if they need to be reversed again!
                    else:
                        np.savetxt(files[m], new_samples, fmt='%.4f')
